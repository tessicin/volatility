...
def _optimize_weights(df: pd.DataFrame, signals_df: pd.DataFrame,
    ...
    # Precompute signal matrix for speed
    mat = signals_df[SIGNAL_KEYS].values.astype(float)
    fwd_vals = df[fwd_col].values if fwd_col in df.columns else None

    def _objective(w_raw):
        w = np.abs(w_raw); s = w.sum()
        if s < 1e-9: return -999
        w = w / s
        score_arr = (mat @ w) * 100
        signal_mask = score_arr >= 50   # use fixed threshold during optimisation
        ret_signal = fwd_vals[signal_mask[:-fwd_days]] if fwd_vals is not None else np.array([])
        ret_all    = fwd_vals
        if len(ret_signal) < 5:
            return -999
        r = ret_signal[~np.isnan(ret_signal)] / 100
        if len(r) < 3: return -999
        if objective == "sharpe":
            sig_ = r.std()
            return r.mean() / sig_ if sig_ > 1e-9 else -999
        elif objective == "avg_return":
            return float(r.mean())
        elif objective == "win_rate":
            return float((r > 0).mean())
        elif objective == "edge":
            r_all = ret_all[~np.isnan(ret_all)] / 100
            return float(r.mean() - r_all.mean()) if len(r_all) > 0 else -999
        return -999

    # Phase 1: random search
    history = []
    best_w = np.ones(len(SIGNAL_KEYS))
    best_val = -999
    ...
    for _ in range(400):
        for i in range(len(best_w)):
            for delta in [step, -step]:
                ...
                candidate[i] = max(0.001, candidate[i] + delta)
                candidate /= candidate.sum()
                val = _objective(candidate)
                if val > best_val:
                    best_val = val
                    best_w = candidate.copy()
        step *= 0.92   # decay step size
