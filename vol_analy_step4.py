...
# IMPORTANT: no "www" (more robust in many networks)
...
def get_options_data(ticker: str, window: float = 0.10, expired: bool = False, sleep_s: float = 0.02):
    ...
    # Ensure numeric openInterest/volume
...
def plot_open_interest(options_data, recent_price, ticker, max_expirations=10):
    if options_data is None or options_data.empty:
        print("No options_data to plot open interest.")
    ...
    fig.update_layout(
        title=f"{ticker} Open Interest by Strike - Spot: {recent_price:.2f}",
        showlegend=True,
        legend_title_text="Expiration Date"
    )
    fig.update_xaxes(title_text="Strike Price")
...
def interpret_open_interest(ticker, overall_avg_oi_calls, overall_avg_oi_puts, calls_data, puts_data):
    ...
    if overall_avg_oi_calls > overall_avg_oi_puts:
        interpretation += "- Call options have higher average open interest (more positioning in calls).\n"
    else:
        interpretation += "- Put options have higher average open interest (more positioning in puts).\n"

    # Highest OI rows (defensive: idxmax can fail if all NaN)
    if calls_data["openInterest"].notna().any():
        highest_oi_call = calls_data.loc[calls_data["openInterest"].idxmax()]
        interpretation += f"- Highest OI (calls): strike {highest_oi_call['strike']} with OI {highest_oi_call['openInterest']}.\n"
    else:
        interpretation += "- Highest OI (calls): N/A (missing OI data).\n"

    if puts_data["openInterest"].notna().any():
        highest_oi_put = puts_data.loc[puts_data["openInterest"].idxmax()]
        interpretation += f"- Highest OI (puts): strike {highest_oi_put['strike']} with OI {highest_oi_put['openInterest']}.\n"
    else:
        interpretation += "- Highest OI (puts): N/A (missing OI data).\n"

    if overall_avg_oi_calls > overall_avg_oi_puts:
        interpretation += "- Higher call OI can indicate more bullish positioning (or call overwriting / structures).\n"
    else:
        interpretation += "- Higher put OI can indicate more hedging / bearish positioning (or put selling / structures).\n"
...
if __name__ == "__main__":
    ...
    # Open interest
    overall_avg_oi_calls, overall_avg_oi_puts, calls_data, puts_data = \
        plot_open_interest(options_data, recent_price, ticker, max_expirations=10)

    if overall_avg_oi_calls is not None:
