...
with tab7:
    ...
    def _price_ta_chart(df, ticker, spot):
        ...
        fig = make_subplots(rows=2, cols=1, shared_xaxes=True,
                            ...
                            subplot_titles=["Prezzo + MA + Bande di Bollinger", "RSI (14)"])
        # BB band fill
        ...
        fig.update_layout(**DARK_LAYOUT, height=480, title=f"{ticker} — Analisi Tecnica (120 giorni)")
...
def _optimize_weights(df: pd.DataFrame, signals_df: pd.DataFrame,
                      fwd_days: int, objective: str = "sharpe",
                      n_iter: int = 800, random_seed: int = 42) -> tuple:
    """
    Two-phase optimisation:
      Phase 1 — random grid search (n_iter random weight vectors)
      Phase 2 — hill climbing from best candidate
    Returns (best_weights dict, best_score float, history list of scores).
    """
    rng = np.random.default_rng(random_seed)
    fwd_col = f"fwd_{fwd_days}"

    # ── Align df and signals_df on a shared, clean index ──────────
    # Join on common index, drop any row where fwd return is NaN
    ...
    if fwd_col not in df_aln.columns:
        # Return equal weights if forward column missing
        return {k: 1.0 / len(SIGNAL_KEYS) for k in SIGNAL_KEYS}, -999.0, []

    # Build a fully-valid mask: no NaN in fwd_col AND not in the last fwd_days rows
    # (last rows have no valid forward return by construction)
    valid_mask = df_aln[fwd_col].notna().values
    valid_mask[-fwd_days:] = False          # last N rows have no future data

    mat      = sig_aln[SIGNAL_KEYS].values.astype(float)[valid_mask]
    fwd_vals = df_aln[fwd_col].values[valid_mask]
    # Baseline: all rows regardless of signal
    fwd_all  = fwd_vals[~np.isnan(fwd_vals)]

    if len(fwd_all) < 10:
        return {k: 1.0 / len(SIGNAL_KEYS) for k in SIGNAL_KEYS}, -999.0, []

    def _objective(w_raw):
        w = np.abs(w_raw)
        s = w.sum()
        if s < 1e-9:
            return -999.0
        w = w / s
        score_arr    = (mat @ w) * 100          # shape: (n_valid,)
        signal_mask  = score_arr >= 50          # same length as mat
        ret_signal   = fwd_vals[signal_mask]    # index-safe: same length
        ret_signal   = ret_signal[~np.isnan(ret_signal)]
        if len(ret_signal) < 5:
            return -999.0
        r = ret_signal / 100.0
        if objective == "sharpe":
            sig_ = r.std()
            return float(r.mean() / sig_) if sig_ > 1e-9 else -999.0
            ...
            return float(r.mean() - base.mean()) if len(base) > 0 else -999.0
        return -999.0

    # Phase 1: random search
    history  = []
    best_w   = np.ones(len(SIGNAL_KEYS)) / len(SIGNAL_KEYS)
    best_val = -999.0
    for _ in range(n_iter):
        w   = rng.dirichlet(np.ones(len(SIGNAL_KEYS)))
        val = _objective(w)
    ...
    # Phase 2: hill climbing with step decay
    step = 0.08
    for _ in range(400):
        improved = False
        for i in range(len(best_w)):
            for delta in [step, -step]:
                ...
                candidate[i] = max(1e-4, candidate[i] + delta)
    ...
    best_dict = {k: float(v) for k, v in zip(SIGNAL_KEYS, best_w / best_w.sum())}
    return best_dict, best_val, history


with tab8:
    st.markdown("### 📉 Backtest Engine v2 — Pesi Configurabili, Ottimizzatore, Multi-Timeframe")
    st.markdown("""
    <div class='insight-box'>
    Testa il <b>Buy Score con pesi personalizzati</b> su dati storici del perpetual Deribit.
    Ogni segnale tecnico ha un peso che puoi regolare manualmente <i>oppure</i> far ottimizzare
    automaticamente via <b>random grid search + hill climbing</b>.
    Analisi disponibile su 4 timeframe (15m · 1H · 4H · 1D) con metriche complete di rischio/rendimento.
    </div>""", unsafe_allow_html=True)
    ...
    # ════════════════════════════════════════════
    #  SECTION B — SIGNAL WEIGHTS
    # ════════════════════════════════════════════
    st.markdown("---")
    st.markdown("#### 🎛️ Pesi dei Segnali")
    st.markdown("""
    <div class='insight-box' style='font-size:12px;'>
    Regola i pesi di ogni segnale (0 = escludi, 10 = peso massimo).
    I pesi vengono normalizzati automaticamente a 1.
    Usa <b>Ottimizza automaticamente</b> per trovare i pesi migliori via grid search + hill climbing.
    </div>""", unsafe_allow_html=True)

    weight_mode = st.radio("Modalità pesi", ["Manuale", "Ottimizzati (grid search + hill climbing)", "Equal Weight"],
                           horizontal=True, key="bt_wmode")
    opt_objective = st.selectbox("Obiettivo ottimizzatore", ["sharpe", "avg_return", "win_rate", "edge"],
                                 key="bt_obj") if weight_mode == "Ottimizzati (grid search + hill climbing)" else "sharpe"

    # Manual weight sliders — always shown (disabled in auto mode)
    w_cols = st.columns(5)
    manual_weights = {}
    slider_disabled = (weight_mode != "Manuale")
    for idx, (key, label) in enumerate(SIGNAL_LABELS.items()):
        col = w_cols[idx % 5]
        manual_weights[key] = col.slider(
            label, 0.0, 10.0, 1.0, 0.5,
            key=f"w_{key}",
            disabled=slider_disabled,
            help=f"Segnale: {key}"
        )

    # Equal weight override
    if weight_mode == "Equal Weight":
        manual_weights = {k: 1.0 for k in SIGNAL_KEYS}

    # ── Threshold ────────────────────────────────────────────────
    bt_threshold = st.slider("Soglia BUY (score ≥)", 30, 80, 60, 5, key="bt_thresh")

    # ════════════════════════════════════════════
    #  SECTION C — DATA + COMPUTE
    # ════════════════════════════════════════════
    st.markdown("---")
    run_bt = st.button("▶ Esegui Backtest", key="bt_run")
    ...
    if run_bt or st.session_state.get("_res_bt_results") is None:
        ...
        if bt_df_raw.empty or len(bt_df_raw) < 80:
            ...
            iv_proxy_bt = float(data["impliedVolatility"].median()) if not data.empty else 70.0
            sigs_df = _compute_raw_signals(bt_df_raw, iv_proxy=iv_proxy_bt)

            # Forward returns on raw df (needed for optimizer)
            for fwd in [3, 7, 14, 30, 60, 90]:
                fp = bt_df_raw["close"].shift(-fwd)
                bt_df_raw[f"fwd_{fwd}"] = (fp / bt_df_raw["close"] - 1) * 100 - (bt_costs / 100)

            if weight_mode == "Ottimizzati (grid search + hill climbing)":
                with st.spinner(f"Ottimizzazione in corso ({opt_objective}) — ~5 sec…"):
                    opt_weights, opt_val, opt_history = _optimize_weights(
                        bt_df_raw, sigs_df, bt_fwd_days, objective=opt_objective,
                        n_iter=900)
    ...
    if bt_result is None or bt_result.empty:
                ...
                if opt_history:
                    ...
                    if h_clean:
                        ...
                        fig_opt.update_layout(**DARK_LAYOUT, height=280,
                            title=f"Curva di ottimizzazione — {opt_objective}",
                            xaxis_title="Tentativo (random phase)", yaxis_title=opt_objective)
        ...
        fig_ts = make_subplots(rows=2, cols=1, shared_xaxes=True,
                               ...
                               subplot_titles=["Buy Score", f"Prezzo {ticker} Perpetual"])
        ...
        fig_ts.update_layout(**DARK_LAYOUT, height=500, showlegend=True,
            title=f"Score & ingresso su {bt_tf}")
        fig_ts.update_yaxes(title_text="Score [0–100]", row=1, col=1, range=[-5, 108])
        fig_ts.update_yaxes(title_text="Prezzo ($)", row=2, col=1)
        ...
        if run_mtf:
            ...
            for tf in ["1D", "4H", "1H"]:
                ...
                if tf_df.empty or len(tf_df) < 60:
                    continue
                tf_sigs = _compute_raw_signals(tf_df, iv_proxy=float(data["impliedVolatility"].median()) if not data.empty else 70.0)
