...
def _equity_sim(bt: pd.DataFrame, fwd_days: int, threshold: float,
                costs_bps: float = 10.0) -> tuple:
    """
    Equity simulation with score-driven entry AND exit:
    - Entry  : score crosses >= threshold (transition from below)
    - Exit   : score drops back below threshold
    - Costs  : costs_bps applied one-way at entry and exit
    - Cash   : factor = 1.0 when not in position
    Returns (strategy_equity np.array, bh_equity np.array), both starting at 1.0.
    """
    close  = bt["close"].values
    scores = bt["score"].values
    n      = len(close)
    cost_one_way = costs_bps / 10_000.0   # e.g. 10bps -> 0.001
    ...
    for i in range(1, n):
        ...
        if not in_pos:
            if scores[i] >= threshold and scores[i - 1] < threshold:
                in_pos = True
                strat[i] = (1.0 + bar_ret) * (1.0 - cost_one_way)
            # else cash: strat[i] stays 1.0
        else:
    ...
    bh = np.ones(n)
    for i in range(1, n):
        bh[i] = close[i] / close[i - 1]

    return np.cumprod(strat), np.cumprod(bh)
...
with tab8:
    ...
    if bt_result is None or bt_result.empty:
        ...
        st.markdown("---")
        st.markdown("#### 📉 Score nel Tempo & Punti di Ingresso/Uscita")
        ...
        # Compute entry / exit bar indices (score-driven, same logic as _equity_sim)
        _scores_arr = bt_valid["score"].values
        _entry_idx  = []   # bars where score crosses up through threshold
        _exit_idx   = []   # bars where score crosses down through threshold
        _in_pos     = False
        for _i in range(1, len(_scores_arr)):
            if not _in_pos:
                if _scores_arr[_i] >= bt_threshold and _scores_arr[_i - 1] < bt_threshold:
                    _entry_idx.append(_i)
                    _in_pos = True
            else:
                if _scores_arr[_i] < bt_threshold:
                    _exit_idx.append(_i)
                    _in_pos = False

        _entry_dates  = bt_valid.index[_entry_idx]
        _exit_dates   = bt_valid.index[_exit_idx]
        _entry_scores = bt_valid["score"].iloc[_entry_idx]
        _exit_scores  = bt_valid["score"].iloc[_exit_idx]
        _entry_prices = bt_valid["close"].iloc[_entry_idx]
        _exit_prices  = bt_valid["close"].iloc[_exit_idx]

        # Also shade "in-position" periods on the price panel
        # Build a boolean series for "in position"
        _in_pos_series = pd.Series(False, index=bt_valid.index)
        _cur_in = False
        for _i in range(1, len(_scores_arr)):
            if not _cur_in and _scores_arr[_i] >= bt_threshold and _scores_arr[_i - 1] < bt_threshold:
                _cur_in = True
            elif _cur_in and _scores_arr[_i] < bt_threshold:
                _cur_in = False
            _in_pos_series.iloc[_i] = _cur_in

        fig_ts = make_subplots(rows=2, cols=1, shared_xaxes=True,
                               row_heights=[0.38, 0.62], vertical_spacing=0.04,
                               subplot_titles=["Buy Score", f"Prezzo {asset_label}"])

        # ── Row 1: Score line + threshold + entry/exit dots ──────
        fig_ts.add_trace(go.Scatter(
            x=bt_valid.index, y=bt_valid["score"], mode="lines",
            line=dict(color="#5468ff", width=1.6), name="Score"), row=1, col=1)
        fig_ts.add_hline(y=bt_threshold,
            line=dict(color="#2ecc71", dash="dash", width=1.2),
            annotation_text=f"Soglia {bt_threshold}", row=1, col=1)
        if len(_entry_dates):
            fig_ts.add_trace(go.Scatter(
                x=_entry_dates, y=_entry_scores,
                mode="markers",
                marker=dict(color="#2ecc71", size=10, symbol="triangle-up",
                            line=dict(color="#ffffff", width=1.2)),
                name=f"Entry (score≥{bt_threshold})"), row=1, col=1)
        if len(_exit_dates):
            fig_ts.add_trace(go.Scatter(
                x=_exit_dates, y=_exit_scores,
                mode="markers",
                marker=dict(color="#e74c3c", size=10, symbol="triangle-down",
                            line=dict(color="#ffffff", width=1.2)),
                name=f"Exit (score<{bt_threshold})"), row=1, col=1)

        # ── Row 2: Price + shaded in-position + entry/exit markers ─
        fig_ts.add_trace(go.Scatter(
            x=bt_valid.index, y=bt_valid["close"], mode="lines",
            line=dict(color="#f0f2ff", width=1.4), name="Prezzo"), row=2, col=1)

        # Shade in-position periods as green background
        _pos_transitions = _in_pos_series.astype(int).diff().fillna(0)
        _starts = bt_valid.index[_pos_transitions == 1].tolist()
        _ends   = bt_valid.index[_pos_transitions == -1].tolist()
        # If still in position at end, close with last date
        if len(_starts) > len(_ends):
            _ends.append(bt_valid.index[-1])
        ...
        # Entry: large green triangle-up on price
        if len(_entry_dates):
            fig_ts.add_trace(go.Scatter(
                x=_entry_dates, y=_entry_prices,
                mode="markers",
                marker=dict(
                    color="#2ecc71", size=14, symbol="triangle-up",
                    line=dict(color="#ffffff", width=1.5)),
                name="Entry price", showlegend=True), row=2, col=1)

        # Exit: large red triangle-down on price
        if len(_exit_dates):
            fig_ts.add_trace(go.Scatter(
                x=_exit_dates, y=_exit_prices,
                mode="markers",
                marker=dict(
                    color="#e74c3c", size=14, symbol="triangle-down",
                    line=dict(color="#ffffff", width=1.5)),
                name="Exit price", showlegend=True), row=2, col=1)
        ...
        fig_ts.update_layout(**DARK_LAYOUT, height=580, showlegend=True,
            title=f"{asset_label} — Score, ingresso e uscita su {bt_tf}  |  "
                  f"{len(_entry_idx)} entries · {len(_exit_idx)} exits")
        ...
        # Use the full bt_valid series (already dropna on score; keep NaN fwd
        # so equity sim works over the whole price history available)
        _eq_df = bt_valid.copy()
        strat_eq, bh_eq = _equity_sim(_eq_df, bt_fwd_days, bt_threshold, float(bt_costs))

        fig_eq = go.Figure()
        fig_eq.add_trace(go.Scatter(
            x=_eq_df.index, y=bh_eq,
            mode="lines", line=dict(color="#6b7394", width=1.8), name="Buy & Hold"))
        fig_eq.add_trace(go.Scatter(
            x=_eq_df.index, y=strat_eq,
            mode="lines", line=dict(color="#5468ff", width=2.5),
            name=f"Strategia Score≥{bt_threshold}"))
        fig_eq.add_hline(y=1, line=dict(color="#f0f2ff", dash="dot", width=1))
        fig_eq.update_layout(**DARK_LAYOUT, height=340,
            title=f"Equity multiplo (base 1.0) — {bt_tf} · costi {bt_costs}bps · uscita quando score<soglia",
